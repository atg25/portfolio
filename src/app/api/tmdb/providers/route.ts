...
export async function GET(req: NextRequest) {
  ...
  try {
    const tmdbRes = await fetch(`https://api.themoviedb.org/3/movie/${id}/watch/providers?api_key=${TMDB_API_KEY}`);
    ...
    const providers = region?.flatrate || region?.ads || region?.rent || region?.buy || [];
